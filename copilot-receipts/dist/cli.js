...
program
    ...
    .description("Generate quirky, shareable receipts for your GitHub Copilot usage")
    .version("1.0.0");
// generate command
program
    .command("generate")
    ...
    .addOption(new Option("-o, --output <format...>", "Output format(s): html, console (comma-separated or repeated)").argParser((value, prev) => {
    const formats = value
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);
...
program
    .command("setup")
    .description("Interactive setup wizard for copilot-receipts (org, token, preferences)")
