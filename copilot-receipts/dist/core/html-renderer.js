...
export class HtmlRenderer {
    ...
    generateHtml(data, receiptText) {
        ...
        const editorRowsHtml = usage.editorBreakdowns
            .map((e) => {
            const rate = e.suggestions_count > 0
                ? formatPercent((e.acceptances_count / e.suggestions_count) * 100)
                : "—";
        ...
        const langRowsHtml = usage.languageBreakdowns
            .map((l) => {
            const rate = l.suggestions_count > 0
                ? formatPercent((l.acceptances_count / l.suggestions_count) * 100)
                : "—";
            return `<div class="line-item">
          ...
          <span>${formatNumber(l.suggestions_count)} suggestions (${rate})</span>
            ...
            .join("\n");
        const chatHtml = usage.totalChatTurns > 0
