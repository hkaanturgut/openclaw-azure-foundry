...
export class DataFetcher {
     ...
     */
    async fetchUsage(org, token, date) {
        const days = await this.fetchRawUsage(org, token);
        if (days.length === 0) {
            throw new Error("No Copilot usage data found. Ensure the organization has Copilot enabled and your token has the required scopes (read:org or manage_billing:copilot).");
        }
        let day;
        if (date) {
            const found = days.find((d) => d.day === date);
            if (!found) {
                throw new Error(`No usage data found for ${date}. Available dates: ${days
                    .map((d) => d.day)
                    .slice(0, 5)
                    .join(", ")}`);
            }
    ...
    async fetchRawUsage(org, token) {
        ...
        if (!response.ok) {
            ...
            if (response.status === 403) {
                throw new Error("Access denied. Your token requires 'read:org' or 'manage_billing:copilot' scope, and the organization must have GitHub Copilot Business/Enterprise enabled.");
            }
            if (response.status === 404) {
                throw new Error(`Organization '${org}' not found, or Copilot Business/Enterprise is not enabled for this organization.`);
            }
     ...
     */
    parseDay(day, org) {
        const acceptanceRate = day.total_suggestions_count > 0
            ? (day.total_acceptances_count / day.total_suggestions_count) * 100
            : 0;
        const lineAcceptanceRate = day.total_lines_suggested > 0
